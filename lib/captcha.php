...
function captcha_show($session_id) {
        ...
        $code=db_query_to_variable("SELECT `captcha` FROM `sessions` WHERE `session`='$session_id_escaped'");
        if($code=='') {
...
function captcha_regenerate($session_id) {
        ...
        db_query("UPDATE `sessions` SET `captcha`='$code_escaped' WHERE `session`='$session_id_escaped'");
        return $code;
}
...
function captcha_check($session_id,$user_code) {
        ...
        $session_id_escaped=db_escape($session_id);
        $real_code=db_query_to_variable("SELECT `captcha` FROM `sessions` WHERE `session`='$session_id_escaped'");
